...
The initial code is set up to run with a small ensemble size to allow you to be able to "Knit" this document. The final run should be conducted with a decently large ensemble (500-5000 depending on what your computer can handle). Since this requires large simulation, turn in the final HTML, not the Rmd, and feel free to answer questions separately (e.g. in email or another doc) so you don't have to run a second time to include your answers. Alternatively, you could run the analysis block-by-block and cut-and-paste the figures into a separate document.
...
for(i in 1:12){
  ...
  ciEnvelope(1:nt,ci[1,],ci[3,],col="lightGrey")
  lines(ci[2,])
  if(varnames[i]=="NEP"){
    lines(-L4$NEE_st_fMDS,col=2)
  }
}
dev.off()

if(FALSE){
  ## KEEP IN MASTER, was used for testing
  plot(-L4$NEE_st_fMDS,output$NEP)
  ...
  GPPm[PAR < 1] = NA
  sel = which(GPPt > 0.1)
  plot(GPPt[sel],GPPm[sel])
  summary(lm(GPPm[sel]~GPPt[sel]-1))

  REt = L4$Reco_st
  REm = (output$Ra+output$Rh)*0.1
  #GPPt[PAR < 1] = NA
  #GPPm[PAR < 1] = NA
  #sel = which(GPPt > 0.1)
  plot(REt,REm)
  summary(lm(REm~REt-1))
}
```
...
plot(Mtime,LAIm.ci[2,],ylab="LAI",xlab="Time",
     ylim=range(c(range(LAIm.ci),range(LAIr,na.rm=TRUE))),type='n')
ciEnvelope(Mtime,LAIm.ci[1,],LAIm.ci[3,],col="lightGrey")
points(Mtime,LAIr)    
...
plot(Mtime,LAIm.ci[2,],ylim=range(c(range(LAIm.ci),range(LAIr,na.rm=TRUE))),
     type='n',ylab="LAI",xlab="Time")
ciEnvelope(Mtime,LAIm.ci[1,],LAIm.ci[3,],col="lightGrey")
ciEnvelope(Mtime,LAIpf[1,],LAIpf[3,],col="lightBlue")
points(Mtime,LAIr)    
for(i in 1:length(LAIr)){
  if(!is.na(QC[i])){
    lines(rep(Mtime[i],2),LAIr[i]+c(-1,1)*LAIr.sd[i])
...
plot(Mtime,LAIm.ci[2,],ylim=range(c(range(LAIm.ci),range(LAIr,na.rm=TRUE))),
     type='n',ylab="LAI",xlab="Time")
ciEnvelope(Mtime,LAIm.ci[1,],LAIm.ci[3,],col="lightGrey")
ciEnvelope(Mtime,LAIpf[1,],LAIpf[3,],col=col.alpha("lightBlue",0.5))
ciEnvelope(Mtime,LAIpr.ci[1,],LAIpr.ci[3,],col=col.alpha("lightGreen",0.5))
points(Mtime,LAIr)    
for(i in 1:length(LAIr)){
  if(!is.na(QC[i])){
    lines(rep(Mtime[i],2),LAIr[i]+c(-1,1)*LAIr.sd[i])
